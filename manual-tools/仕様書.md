...
- 入力データは、章単位に分割されたテキストファイル（拡張子 `.txt` / `.md` / `.json`）を用いる。
- 各マニュアルは `manuals/` 以下の 1 サブフォルダに対応する。
  - 例: マニュアル名「給付金編」 → `manuals/給付金編/`
- 目次は同一フォルダ内の `00_目次.json` を「正」とする。
  - 旧来の `00_目次.txt` が存在していてもよいが、サーバー側では参照しない。
- バックエンドサーバーは Python 製の FastAPI アプリケーションとし、ローカルの HTTP API を提供する。
- クライアントは Claude Desktop を想定し、MCP ローカルツール（`manual-tools`）を経由して FastAPI バックエンドの HTTP API を利用する。
...
- 本仕様書および MCP ブリッジサーバー（`manual-tools-mcp`）の Zod `outputSchema` を、`manual-tools` の HTTP/JSON API 契約の唯一の正とする。
  - FastAPI バックエンドの実装は、これと常に一致させる。
...
- 各マニュアルフォルダには少なくとも以下が存在する前提とする。
  ...
  - 章本文のテキストファイル群: 複数の `.txt` / `.md` / `.json` ファイル
...
- `toc` 配列の各要素は、最低限次のフィールドを持つ。
  ...
  - `file`: 章本文に対応するテキストファイル名（`.txt` / `.md` / `.json`）。実体は `manuals/{manual_name}/{file}` に存在する前提。
...
- 目的: Claude（MCP）から利用可能な「構造理解可能なマニュアル参照 API」を提供すること
- 利用形態:
  - Claude Desktop（UI） → MCP ローカルツール `manual-tools`（Node ブリッジ） → FastAPI サーバー（ローカル HTTP バックエンド）
- FastAPI バックエンドの動作環境:
  - ローカル PC 上で FastAPI アプリケーションとして起動
  - 必要なときのみ起動すればよい（常時稼働不要）
- データ層:
  - ローカルフォルダ `manuals/` 以下の `.txt` / `.md` / `.json` ファイル群と、各フォルダの `00_目次.json`
- FastAPI バックエンドのベース URL:
  - `/`（全エンドポイントはルート直下に配置）
- FastAPI バックエンドのバインド先:
  - `http://127.0.0.1:{PORT}`（外部からのアクセスは前提にしない）
- MCP ブリッジサーバー:
  ...
  - `resolve_reference` と `get_outline` は HTTP では利用可能だが、MCP ツールとしては未公開
  - ツールの入力と出力の形式は Zod を用いて定義し、FastAPI バックエンドの JSON 形式と整合するように設計する
  - FastAPI から返ってきた JSON は `structuredContent` とテキスト（JSON 文字列）として Claude に返す
  - 出力形式が Zod スキーマと一致しない場合、MCP SDK によって Output validation error（エラーコード `-32602`）が発生するため、FastAPI 側のレスポンス形式と MCP ブリッジ側の `outputSchema` は常に同期しておく
...
- 主な責務
  ...
  - 章タイトルに基づく参照解決（`resolve_reference`）

### 4.3 services

- 役割: ドメインロジック（検索・例外抽出・ToC 整形など）
- 主な責務
  - ToC の整形
    - フラット構造（単純な章一覧）
...
- FastAPI のルーターレイヤー
- HTTP エンドポイント定義（`/list_manuals`, `/get_toc`, `/list_sections`, `/get_section`, `/search_text`, `/find_exceptions`, `/resolve_reference`, `/get_outline` 等）
- 依存性注入（`Depends`）によりリポジトリやサービスを取得
...
- `manuals_root`:
  - マニュアルルートディレクトリ名（デフォルトは `manuals`）
- `toc`:
  - `source`: 目次ソース。現状は `json_only` を前提
  - `path_pattern`: ToC JSON のパステンプレート（例: `manuals/{manual}/00_目次.json`）
  - `hierarchical_default`: ToC の階層表現を既定で使うか（現状は `false`）
  - `use_loc`: 目次が `children` を持つ場合の扱いに関する将来用スイッチ（現状は `false`）
  - `overrides.enabled`: ToC 上書き機能の有効化（現状は `false`）
- `validation_mode`:
  - `relaxed` / `strict`（現状は `relaxed` のみ運用）
- `logging`:
  - `level`: ログレベル（例: `INFO`）
...
- `manuals_root` 直下のサブディレクトリを列挙し、そのフォルダ名を `manual_name` として採用する。
...
- パス: `/list_manuals`
- クエリ引数: なし
- 戻り値: マニュアル名の配列（例: `["給付金編", "コンプライアンスマニュアル"]`）
- 概要:
  - 登録済みマニュアル一覧を返す
...
- 戻り値:
  - `id`: 章 ID
  - `children`: 小見出し配列（ToC の `children` を返す。`null` の場合は空配列）
- 概要:
  - ToC の `children` 情報を取得する軽量 API
  - MCP ツールとしては未公開
...
- リクエストボディ:
  ...
  - `mode`（任意。`plain` / `regex` / `loose`。省略時は `regex`）
  - `limit`（任意。最大件数。省略時は `10`）
  - `case_sensitive`（任意。省略時は `false`）
- 戻り値:
...
### 7.8 `resolve_reference`

- HTTP: `POST`
- パス: `/resolve_reference`
- 状態: HTTP エンドポイントは実装済み、MCP ツールは未公開
- リクエスト:
  - `manual_name`
  - `ref_text`（例: 「第3章 重大手技を参照」など）
- 戻り値:
  - `target_section`（推定される `section_id`、見つからない場合は `null`）
- 概要:
...
- 概要: MCP 側が「サーバーが起動しているか」を確認するためのヘルスチェックエンドポイント
...
- 検索対象は指定マニュアルの各セクション本文。
  - `section_id` が指定されていればその章のみ対象
  - 指定が無ければ、`list_sections` 相当の全章を対象に順次検索
- 結果は上位 `limit` 件まで返す（`limit` 未指定時は `10`）。
...
- パターンコンパイルに失敗した場合（不正な正規表現など）は例外を外に出さず、`plain` と同等のプレーン一致にフォールバックする。
...
- 内部的な挙動のイメージ:
  - クエリ文字列の各文字の間に空白（半角・全角）、中点、スラッシュ（全角・半角）、各種ハイフンなどを許容する正規表現を自動生成する。
  - 例として「帝王切開」を指定した場合、
    - 「帝 王 切 開」
    - 「帝・王/切-開」
    - 「帝　王　切　開」
    などの表記揺れもマッチするようなパターンを使用する。
...
- マッチした行について、その前後 1 行ずつをまとめて 1 つの `text` として返す。
- 返却要素の構造:
